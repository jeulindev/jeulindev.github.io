...
new (function() {
	...
	ext.setMotor1 = function(param1, param2, callback) {
		if (!device) 
		{
			...
			myRooby.write(device, {
				type : 'write',
				sel	: [0x00, 0x65, 0x86, parseInt(val), (motor1_dir_rot==-1)?0xff:0, (motor1_dir_rot==-1)?0:1, 0x00, 0x00, 0x00]
			}, callback);			
		}
		return true;
	}
	...
	ext.setMotor2 = function(param1, param2, callback) {
		...
		try {callback(); } catch(err) {console.log(err.message); }
			...
			myRooby.write(device, {
				type : 'write',
				sel	: [0x00, 0x65, 0x85,  parseInt(val),(motor2_dir_rot==-1)?0xff:0, (motor2_dir_rot==-1)?0:1, 0x00, 0x00, 0x00]
			}, callback);
		}
		return true;
	}
